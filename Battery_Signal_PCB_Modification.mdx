...
Pin 7 on the Arduino MKR 1010 is used as a CS Pin for the SPI interface which is not obvious when looking at the official pinout. The Freelancer did not use the standard pins with the can bus circuit which led to the issues we experienced. Pin 7 was assinged as the SPI CS pin and also as the cs pin for the MM